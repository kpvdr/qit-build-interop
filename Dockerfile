...
RUN dnf -y install sudo unzip wget git gcc-c++ make cmake maven python2-devel python38-devel procps-ng && \
    def -y install https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm && \
